...
#define CMD_MAX_LEN 4096
  ...
  class tair_client : public INairServerCoreHandler{
    public:
      tair_client();
      ~tair_client();
	...
	private:
	  NairServiceCore* _naircore_object;
	  void OnNairRemoteJoined(int remotesocket, int remotecount);
	  void OnNairRemoteUnJoined(int remotesocket, int remotecount);
	  void OnNairPutRequest(unsigned int area, unsigned int expired, unsigned int version, char* key, char* value, unsigned short& code);
	  void OnNairGetRequest(unsigned int area, char* key, unsigned int& version, char*& value, unsigned short& code);
	  void OnNairGetsRequest(unsigned int area, vector<NairKey*>& keyvalues, unsigned short& code);
	  void OnNairRemoveRequest(unsigned int area, char* key, unsigned short& code);
	  void OnNairIncrRequest(unsigned int area, unsigned int expired, unsigned int value, unsigned int defaultvalue, unsigned int& returnvalue, char* key, unsigned short& code);
	private:
	  	void MakeKeys(char* pkey, int keycount, vector<char*>& keys);
		void MakeValues(char* pvalue, vector<char*>& values);
		int  GetKeysCount(char* pValue);
		void GetKeysData(char* key, vector<char*>& chkeys);
		int  AppendKeysData(char* key, unsigned int expired, int keycount, int valuesize, vector<char*> pchkeys);
		void RemoveKeysData(char* key);
