...
namespace tair {
   ...
   tair_client::tair_client()
   {
	  ...
	  _naircore_object = new NairServiceCore(static_cast<INairServerCoreHandler*>(this));
   }

   tair_client::~tair_client()
   {
	  if(_naircore_object != NULL)
	  {
		_naircore_object->Close();
		delete _naircore_object;
	    _naircore_object = NULL;
	  }
	...
	int tair_client::GetKeysCount(char* pValue)
	{
		int keycount = 0;
		if(pValue != NULL)
		{
			...
			char *avalue = NULL;
			int pvaluesize = 0;
			char* pvalue = canonical_key(v, &avalue, &pvaluesize);
			data_entry entrydata(pvalue, pvaluesize, false);
			int maxsize = TAIR_MAX_DATA_SIZE-50;
			int nsize = entrydata.get_size();
			if(nsize > maxsize)
			{
				keycount = nsize / maxsize;
				if(nsize % maxsize > 0)
					keycount++;
			}
			else
	...
	void tair_client::MakeKeys(char* pkey, int keycount, vector<char*>& keys)
	{
		int nlen = strlen(pkey) + 10;
		for(int i = 0; i < keycount; i++)
		{
			char* ppkey = new char[nlen+1];
			char* tmpkey = ppkey;
			memset(ppkey, 0, nlen+1);
			strncpy(ppkey, pkey, strlen(pkey));
			ppkey += strlen(pkey);
			char pnum[5] = {0}; //四位数+\0 
			itoa(i, pnum);
			int numlen = strlen(pnum);
			strncpy(ppkey, pnum, numlen);
			ppkey += numlen;
			strncpy(ppkey, "<?#?>", 5);
	...
	void tair_client::MakeValues(char* pvalue, vector<char*>& values)
	{
		int totalsize = strlen(pvalue);
		char* tmpvalue = pvalue;
		while(totalsize > 0)
		{
			int maxsize = TAIR_MAX_DATA_SIZE-50;
			int datasize = (totalsize > maxsize ? maxsize : totalsize);
			char* pdata = new char[datasize+1];
			memset(pdata, 0, datasize+1);
			memcpy(pdata, tmpvalue, datasize);
	...
	void tair_client::GetKeysData(char* key, vector<char*>& chkeys)
	{
		char* k = key;
		char *akey = NULL;
		int pkeysize = 0;
		char *pkey = canonical_key(k, &akey, &pkeysize);
		data_entry entrykey(pkey, pkeysize, false);
		data_entry *entrydata = NULL;
		int ret = client_helper.get(50, entrykey, entrydata);
		if (ret == TAIR_RETURN_SUCCESS) 
		{
			string keys;
			keys.reserve(10240);
			keys = entrydata->get_data();
			int nlen = keys.length();
			while(nlen > 0)
			{
				int n = keys.find("<?#?>");
				if(n <= 0)
					break;
				string item = keys.substr(0, n + 5);
				int itemlen = item.length();
				char* pchkey = new char[itemlen+1];
				memset(pchkey, 0, itemlen+1); 
				memcpy(pchkey, item.c_str(), itemlen);
				chkeys.push_back(pchkey);
				keys = keys.substr(n + 5, nlen - n + 1);
				nlen -= n + 5;
			}
			delete entrydata;
		}
	...
	int tair_client::AppendKeysData(char* key, unsigned int expired, int keycount, int valuesize, vector<char*> pchkeys)
	{
		char* k = key;
		int keydatasize = 0;
		for(vector<char*>::iterator it = pchkeys.begin(); it != pchkeys.end(); it++)
			keydatasize += strlen(*it);
			
		char* pkeydata = new char[keydatasize+1];
		memset(pkeydata, 0, keydatasize+1);
		char* ptmpdata = pkeydata;
		for(vector<char*>::iterator it = pchkeys.begin(); it != pchkeys.end(); it++)
		{
			int itsize = strlen(*it);
			memcpy(pkeydata, *it, itsize);
			pkeydata += itsize;
		}
		...
		char *avalue = NULL;
		int pvaluesize = 0;
		char* pvalue = canonical_key(ptmpdata, &avalue, &pvaluesize);
		data_entry entrydata(pvalue, pvaluesize, false);
		int ret = client_helper.put(50, entrykey, entrydata, expired, 0);
		
		delete[] ptmpdata;
		return ret;
	}
	
	void tair_client::RemoveKeysData(char* key)
	{
		char* k = key;
		char *akey = NULL;
		int pkeysize = 0;
		char *pkey = canonical_key(k, &akey, &pkeysize);
		data_entry entrykey(pkey, pkeysize, false);
		client_helper.remove(50, entrykey);
		if (akey) free(akey);
	}

   void tair_client::OnNairRemoteJoined(int remotesocket, int remotecount)
   {
   }
   void tair_client::OnNairRemoteUnJoined(int remotesocket, int remotecount)
   {
   }  
   
   void tair_client::OnNairPutRequest(unsigned int area, unsigned int expired, unsigned int version, char* key, char* value, unsigned short& code)
   {
	  RemoveKeysData(key); //先清除原有key库中存在的子key与所有子key的数据，确保重新添加当前key信息
	  int keycount = GetKeysCount(value);
	  if(keycount > 1)
	  {
	    code = NAIR_FAILED;
		vector<char*> chkeys;
		MakeKeys(key, keycount, chkeys);
		if(chkeys.empty() == false)
		{
			int valuesize = strlen(value); 
			int ret = AppendKeysData(key, expired, keycount, valuesize, chkeys); //key库中添加key信息
			if(ret == 0)
			{
				vector<char*> chvalues;
				MakeValues(value, chvalues);
				if(chkeys.size() == chvalues.size())
				{
					for(int i = 0; i < (int)chkeys.size(); i++)
					{
						char *akey = NULL;
						int pkeysize = 0;
						char *pkey = canonical_key(chkeys[i], &akey, &pkeysize);
						data_entry entrykey(pkey, pkeysize, false);
						
						char *avalue = NULL;
						int pvaluesize = 0;
						char* pvalue = canonical_key(chvalues[i], &avalue, &pvaluesize);
						data_entry entrydata(pvalue, pvaluesize, false);
						
						ret = client_helper.put(area, entrykey, entrydata, expired, version);
						if (akey) free(akey);
						if(avalue) free(avalue);
						if(ret != 0)
						{
				...
				for(vector<char*>::iterator it_chvalue = chvalues.begin(); it_chvalue != chvalues.end(); it_chvalue++)
					delete[] *it_chvalue;
				chvalues.clear();
			}
		}
		for(vector<char*>::iterator it_chkey = chkeys.begin(); it_chkey != chkeys.end(); it_chkey++)
			delete[] *it_chkey;
		chkeys.clear();
	  }
	  else
	  {
		char *akey = NULL;
		int pkeysize = 0;
		char *pkey = canonical_key(key, &akey, &pkeysize);
		data_entry entrykey(pkey, pkeysize, false);
      
		char *avalue = NULL;
		int pvaluesize = 0;
		char* pvalue = canonical_key(value, &avalue, &pvaluesize);
		data_entry entrydata(pvalue, pvaluesize, false);
		int ret = client_helper.put(area, entrykey, entrydata, expired, version);
		code = (ret == 0 ? NAIR_SUCCESSED : NAIR_FAILED );
		if (akey) free(akey);
		if(avalue) free(avalue);
	  }
   }
   
   void tair_client::OnNairGetRequest(unsigned int area, char* key, unsigned int& version, char*& value, unsigned short& code)
   {
	  vector<char*> chkeys;
	  GetKeysData(key, chkeys);
	  if(chkeys.empty() == false)
	  {
		version = 0;
		code = NAIR_FAILED;
		vector<NairKey*> nkeys;
		for(vector<char*>::iterator it_key = chkeys.begin(); it_key != chkeys.end(); it_key++)
		{
			NairKey* pnkey = new NairKey(*it_key);
			nkeys.push_back(pnkey);
		}
		vector<data_entry*> keys;
		for(vector<NairKey*>::iterator it = nkeys.begin(); it != nkeys.end(); it++)
		{
			char *akey = NULL;
			int pkeysize = 0;
			char *pkey = canonical_key((char*)(*it)->GetKey(), &akey, &pkeysize);
		...
		{	
			...
			for ( ; mit != datas.end(); ++mit)
			{
				for(vector<NairKey*>::iterator it = nkeys.begin(); it != nkeys.end(); it++)
				{
					if(strcmp((char*)(*it)->GetKey(), mit->first->get_data()) == 0)
					{
						int size = mit->second->get_size();
						char* pvalue = new char[size+1];
						memset(pvalue, 0, size+1);
						memcpy(pvalue, mit->second->get_data(), size);
						(*it)->SetValue(pvalue);
						datasize += size;
						break;
					}
			...
			value = new char[datasize+1];
			memset(value, 0, datasize+1);
			char* p = value;
			for(vector<NairKey*>::iterator it = nkeys.begin(); it != nkeys.end(); it++)
			{
				int vsize = (*it)->GetValueSize();
				memcpy(p, (char*)(*it)->GetValue(), vsize);
				p += vsize;
			}
		}
	  ...
	  else
	  {
		...
		data_entry entrykey(pkey, pkeysize, false);
		data_entry *entrydata = NULL;
		int ret = client_helper.get(area, entrykey, entrydata);
		if (ret != TAIR_RETURN_SUCCESS) 
		{
			code = NAIR_FAILED;
		} else if (entrydata != NULL) {
			code = NAIR_SUCCESSED;
			version = entrydata->get_version();
			int len = entrydata->get_size();
			value = new char[len+1];
			memset(value, '\0', len+1);
			strncpy(value, entrydata->get_data(), len);
			/*
			format is 1:
			char *p = NULL;
			p = entrydata->get_data();
			if(p != NULL)
			{
				p += 2;
				int len = entrydata->get_size()-2;
				value = new char[len+1];
				memset(value, '\0', len+1);
				strncpy(value, p, len);
			}*/
   ...
   void tair_client::OnNairGetsRequest(unsigned int area, vector<NairKey*>& keyvalues, unsigned short& code)
   {
	  code = NAIR_FAILED;
	  if(keyvalues.size() <= 0)
		return;
      ...
      {
		vector<char*> chkeys;
		GetKeysData((char*)(*it)->GetKey(), chkeys);
		if(chkeys.empty() == true) //先筛选无子key的key数据
		{
			char *akey = NULL;
			int pkeysize = 0;
			char *pkey = canonical_key((char*)(*it)->GetKey(), &akey, &pkeysize);
		...
		{
			int nmkeysize = (*it)->GetKeySize();
			char* pmkey = new char[nmkeysize+1];
			memset(pmkey, 0, nmkeysize+1);
			memcpy(pmkey, (*it)->GetKey(), nmkeysize);
			mkeys.push_back(pmkey); //添加有子key的key数据
		}
		
		for(vector<char*>::iterator it_k = chkeys.begin(); it_k != chkeys.end(); it_k++)
			delete[] *(it_k);
		chkeys.clear();
      }
		...
		if (ret == TAIR_RETURN_SUCCESS || ret == TAIR_RETURN_PARTIAL_SUCCESS)
		{
			...
			for ( ; mit != datas.end(); ++mit)
			{
				for(vector<NairKey*>::iterator it = keyvalues.begin(); it != keyvalues.end(); it++)
				{
					if(strcmp((char*)(*it)->GetKey(), mit->first->get_data()) == 0)
					{
						int size = mit->second->get_size()-1; //去掉结尾符号
						int nsize = size + NAIR_SPLIT_VALUE_SIZE;
						char* pvalue = new char[nsize+1];
						memset(pvalue, 0, nsize+1);
						memcpy(pvalue, mit->second->get_data(), size);
						memcpy(pvalue+size, NAIR_SPLIT_VALUE_CHR, NAIR_SPLIT_VALUE_SIZE); //追加分隔符
						(*it)->SetValue(pvalue);
						/*
						char* pvalue = new char[size+1];
						memset(pvalue, 0, size+1);
	  ...
	  code2 = NAIR_FAILED;
	  for(vector<char*>::iterator it_mkey = mkeys.begin(); it_mkey != mkeys.end(); it_mkey++)
	  {
		vector<char*> chkeys;
		GetKeysData((*it_mkey), chkeys);
		vector<NairKey*> nkeys;
		for(vector<char*>::iterator it_key = chkeys.begin(); it_key != chkeys.end(); it_key++)
		{
			NairKey* pnkey = new NairKey(*it_key);
		...
		{		
			...
			char* value = new char[datasize + NAIR_SPLIT_VALUE_SIZE + 1];
			memset(value, 0, datasize + NAIR_SPLIT_VALUE_SIZE + 1);
			char* p = value;
			for(vector<NairKey*>::iterator it = nkeys.begin(); it != nkeys.end(); it++)
			{
				int vsize = (*it)->GetValueSize();
			...
			memcpy(p, NAIR_SPLIT_VALUE_CHR, NAIR_SPLIT_VALUE_SIZE); //结尾分割标记
			for(vector<NairKey*>::iterator it = keyvalues.begin(); it != keyvalues.end(); it++)
			{
				if(strcmp((char*)(*it)->GetKey(), (*it_mkey)) == 0)
				{
					(*it)->SetValue(value);
					break;
				}
	  ...
	  if(code1 == NAIR_SUCCESSED || code2 == NAIR_SUCCESSED)
		code = NAIR_SUCCESSED;
   }
   
   void tair_client::OnNairRemoveRequest(unsigned int area, char* key, unsigned short& code)
   {
	  vector<char*> chkeys;
	  GetKeysData(key, chkeys);
	  if(chkeys.empty() == false)
	  {
		RemoveKeysData(key);
		vector<data_entry*> keys;
		for(vector<char*>::iterator it = chkeys.begin(); it != chkeys.end(); it++)
		{
			char *akey = NULL;
			int pkeysize = 0;
			char *pkey = canonical_key(*it, &akey, &pkeysize);
			data_entry* key = new data_entry(pkey, pkeysize, false);
			keys.push_back(key);
			if (akey) free(akey);
		}
	  ...
	  else
	  {
		...
		char *pkey = canonical_key(key, &akey, &pkeysize);
		data_entry entrykey(pkey, pkeysize, false);
		int ret = client_helper.remove(area, entrykey);
		code = (ret == 0 ? NAIR_SUCCESSED : NAIR_FAILED );
		if (akey) free(akey);
	  }
   }
   
   void tair_client::OnNairIncrRequest(unsigned int area, unsigned int expired, unsigned int value, unsigned int defaultvalue, unsigned int& returnvalue, char* key, unsigned short& code)
   {
	  char *akey = NULL;
      int pkeysize = 0;
      char *pkey = canonical_key(key, &akey, &pkeysize);
      data_entry entrykey(pkey, pkeysize, false);
      int retCount = 0;
	  int ret = client_helper.add_count(area, entrykey, (int)value, &retCount, (int)defaultvalue, (int)expired);
	  if (ret != TAIR_RETURN_SUCCESS) {
         code = NAIR_FAILED;
      } else {
	     code = NAIR_SUCCESSED;
		 returnvalue = (unsigned int)retCount;
	  }
	  if (akey) free(akey);
	  return;
   }
   ...
   char * tair_client::parse_cmd_line(int argc, char *const argv[])
   {
	int opt;
   ...
   {
      bool done = true;
      client_helper.set_timeout(nair_timeout);
	  ...
	  if(_naircore_object != NULL)
		 _naircore_object->Start(nair_port);
...
{
   char * config_file = _globalClient.parse_cmd_line(argc, argv);
   if(config_file == NULL) {
     _globalClient.print_usage(argv[0]);
     return EXIT_FAILURE;
   }
   ...
   char* nair_env = getenv("NAIR");
   if(nair_env == NULL) {
	 nair_env = (char*)"DEFAULT";
   }

   _globalClient.nair_port = TBSYS_CONFIG.getInt(nair_env, NAIR_PORT, NAIR_DEFAULT_PORT);
   _globalClient.nair_timeout = TBSYS_CONFIG.getInt(nair_env, NAIR_TIMEOUT, 15000);
   _globalClient.server_addr = TBSYS_CONFIG.getString(nair_env, NAIR_TAIR_MASTER_CONFIGSERVER, "127.0.0.1:8199");
   _globalClient.slave_server_addr = TBSYS_CONFIG.getString(nair_env, NAIR_TAIR_SLAVE_CONFIGSERVER, "127.0.0.1:8299");
   _globalClient.group_name =  (char*)TBSYS_CONFIG.getString(nair_env, NAIR_TAIRGROUP, "group_1");
   _globalClient.sz_log_level = TBSYS_CONFIG.getString(nair_env, NAIR_LOG_LEVEL, "info");
   _globalClient.is_config_server = true;
   TBSYS_LOGGER.setLogLevel(_globalClient.sz_log_level);

   if(daemon_init() == -1)
   {
		puts("can't fork self\n");
		exit(0);
   }
   ...
   while(1) { //连接tairconfigserver失败后将反复连接，直到连接成功.
